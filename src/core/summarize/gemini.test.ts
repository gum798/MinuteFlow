import { summarizeWithGemini, verifyGeminiKey } from './gemini'
...
describe('verifyGeminiKey', () => {
  test('빈 키는 요청 없이 empty', async () => {
    const fetchFn = vi.fn()
    const s = await verifyGeminiKey('  ', fetchFn as unknown as typeof fetch)
    expect(s).toMatchObject({ ok: false, kind: 'empty' })
    expect(fetchFn).not.toHaveBeenCalled()
  })

  test('정상 키는 valid + 모델 수·flash 접근·입력 한도를 돌려준다', async () => {
    const fetchFn = vi.fn(async () => new Response(JSON.stringify({
      models: [
        { name: 'models/gemini-3.5-flash', inputTokenLimit: 1048576, supportedGenerationMethods: ['generateContent'] },
        { name: 'models/gemini-3.5-pro', inputTokenLimit: 2097152 },
      ],
    }), { status: 200 }))
    const s = await verifyGeminiKey('AIza_ok', fetchFn as unknown as typeof fetch)
    // models.list(GET)로 확인 — 요약 할당량 미소모
    const [url, init] = fetchFn.mock.calls[0] as unknown as [string, RequestInit]
    expect(url).toContain('/v1beta/models')
    expect((init.headers as Record<string, string>)['x-goog-api-key']).toBe('AIza_ok')
    expect(s).toMatchObject({ ok: true, kind: 'valid', modelCount: 2, hasFlash: true, flashInputLimit: 1048576 })
  })

  test('400은 invalid, 403은 no-permission, 429는 rate-limited', async () => {
    const mk = (status: number) => vi.fn(async () => new Response('{}', { status }))
    expect(await verifyGeminiKey('k', mk(400) as unknown as typeof fetch)).toMatchObject({ ok: false, kind: 'invalid' })
    expect(await verifyGeminiKey('k', mk(403) as unknown as typeof fetch)).toMatchObject({ ok: false, kind: 'no-permission' })
    expect(await verifyGeminiKey('k', mk(429) as unknown as typeof fetch)).toMatchObject({ ok: false, kind: 'rate-limited' })
  })

  test('fetch throw는 network-error', async () => {
    const fetchFn = vi.fn(async () => { throw new TypeError('Load failed') })
    expect(await verifyGeminiKey('k', fetchFn as unknown as typeof fetch)).toMatchObject({ ok: false, kind: 'network-error' })
  })
})
