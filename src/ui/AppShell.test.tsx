...
test('처리 작업이 도는 중이면 진행 표시가 뜨고, 새 버전이 준비돼도 새로고침하지 않는다', () => {
  mockJobsRef.current = [{ meetingId: 'm', kind: 'retranscribe', status: '재전사 중… (1/2)' }]
  const sw = mockServiceWorker(true)
  renderShell()
  expect(screen.getByText(/재전사 중… \(1\/2\)/)).toBeInTheDocument() // 어느 화면에서든 진행 표시
  act(() => sw.fire('controllerchange'))
  expect(reloadPage).not.toHaveBeenCalled() // 처리 중 작업이 죽지 않게 자동 새로고침 보류
  expect(screen.getByText(/끝나면 자동 적용/)).toBeInTheDocument()
})

test('녹음 중에도 진행 중인 작업이 상단에 함께(순서대로) 표시된다', () => {
  ;(getRecordingState as Mock).mockReturnValue({ ...IDLE, phase: 'recording', meetingId: 'rec', elapsedSec: 27 })
  mockJobsRef.current = [{ meetingId: 'm', kind: 'retranscribe', status: '재전사 중…' }]
  renderShell()
  expect(screen.getByText(/녹음 중 · 00:27/)).toBeInTheDocument()   // 녹음
  expect(screen.getByText(/재전사 중…/)).toBeInTheDocument()        // + 작업이 함께 보인다
})
