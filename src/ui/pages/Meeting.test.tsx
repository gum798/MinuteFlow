...
test('부가 여럿인 그룹에서 화자 구분을 누르면 마지막 부만이 아니라 전 부가 통합 화자 구분된다', async () => {
  ...
  // 두 부 모두 세그먼트에 전역 화자(SPK1)가 저장될 때까지 기다린다 — 마지막 부만이 아님.
  // (extract 호출 시점과 applySpeakers 완료 사이의 경합을 피하기 위해 DB 상태를 직접 폴링)
  await waitFor(async () => {
...
test('두 화자를 연달아 이름 변경하면 둘 다 저장된다(변경 전 상태로 덮어쓰기 방지)', async () => {
  ...
  await waitFor(() => screen.getByText('SPK1'))
  await userEvent.click(screen.getByText('SPK1'))
  await userEvent.type(await screen.findByRole('textbox', { name: '새 화자 이름' }), 'Alice')
  await userEvent.click(screen.getByRole('button', { name: '저장' }))
  await waitFor(() => expect(screen.getByText('Alice')).toBeInTheDocument())
  await userEvent.click(screen.getByText('SPK2'))
  await userEvent.type(await screen.findByRole('textbox', { name: '새 화자 이름' }), 'Bob')
  await userEvent.click(screen.getByRole('button', { name: '저장' }))
