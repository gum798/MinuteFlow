...
test('팝업에서 기존 화자 이름 버튼을 누르면 그 이름으로 지정된다', async () => {
  const m = await createMeeting()
  // 한 세그먼트에 SPK1, 그리고 다른 라벨(SPK2)엔 이미 이름을 붙여둔다 → 기존 이름 버튼으로 노출돼야 한다.
  ...
  expect((await db.meetings.get(m.id))?.speakerNames?.SPK1).toBe('박대리')
})
