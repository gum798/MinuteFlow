...
import { createMeeting, finishMeeting, appendSegment, appendAudioChunk, listMeetings, saveSummary, updateSpeakerNames, markGroupFirstPart, getSummaries } from '../../core/store/meetings'
import { saveSettings, loadSettings } from '../../core/settings'
import AppShell from '../AppShell'
import Home from './Home'
import MeetingPage from './Meeting'
import { __resetJobsForTests } from '../../core/jobs'
import { MAX_PROCESS_SEC } from '../../core/meetingActions'
...
test('배지를 클릭하고 팝업에 이름을 입력해 저장하면 표시 이름이 바뀐다', async () => {
  ...
  // 배지 클릭 → 팝업 → 입력창에 타이핑 → 저장
  await userEvent.click(screen.getByText('SPK1'))
  const input = await screen.findByRole('textbox', { name: '새 화자 이름' })
  await userEvent.type(input, '김팀장')
  await userEvent.click(screen.getByRole('button', { name: '저장' }))
  await waitFor(() => expect(screen.getByText('김팀장')).toBeInTheDocument())
  expect((await db.meetings.get(m.id))?.speakerNames?.SPK1).toBe('김팀장')
})

test('팝업에서 기존 화자 이름을 선택하면 같은 사람으로 병합된다(내부 라벨까지)', async () => {
  const m = await createMeeting()
  // SPK1에 발화 하나, 다른 라벨(SPK2)엔 이미 '박대리' 이름을 붙여둔다 → 기존 이름 버튼으로 노출된다.
  await appendSegment({ meetingId: m.id, startSec: 0, endSec: 5, text: '첫 발언입니다', source: 'whisper', isFinal: true, speaker: 'SPK1' })
  await updateSpeakerNames(m.id, { SPK2: '박대리' })
  await finishMeeting(m.id, 60)
  renderPage(m.id)
  await waitFor(() => screen.getByText('SPK1'))
  await userEvent.click(screen.getByText('SPK1'))
  // 팝업 안의 기존 이름 '박대리'를 선택 = "SPK1은 박대리와 같은 사람"
  await userEvent.click(await screen.findByRole('button', { name: '박대리' }))
  await waitFor(() => expect(screen.getByText('박대리')).toBeInTheDocument())
  // 발화의 내부 라벨이 SPK1 → SPK2(박대리)로 병합되고, SPK1 이름 매핑은 사라진다.
  const segs = await db.transcriptSegments.where('meetingId').equals(m.id).toArray()
  expect(segs.every(s => s.speaker === 'SPK2')).toBe(true)
  const names = (await db.meetings.get(m.id))?.speakerNames
  expect(names?.SPK2).toBe('박대리')
  expect(names?.SPK1).toBeUndefined()
})

test('너무 긴 녹음은 자동 정리 시 재전사·화자 구분을 건너뛰고 요약만 하며, 건너뜀을 토스트로 알린다', async () => {
  saveSettings({ geminiApiKey: 'k' })
  const m = await createMeeting()
  await appendSegment({ meetingId: m.id, startSec: 0, endSec: 5, text: '실시간 자막으로 남은 회의 내용입니다', source: 'webspeech', isFinal: true })
  await appendAudioChunk(m.id, 0, new Blob(['aud']), 'audio/webm')
  await finishMeeting(m.id, MAX_PROCESS_SEC + 1) // 상한 초과(예: 16시간)
  vi.spyOn(window, 'confirm').mockReturnValue(true)
  transcribeMock.mockClear()
  diarizeMock.mockClear()
  renderApp(m.id)
  await waitFor(() => screen.getByRole('button', { name: /자동 정리/ }))
  await userEvent.click(screen.getByRole('button', { name: /자동 정리/ }))
  // 건너뜀 안내 토스트가 어느 화면에서든(AppShell) 뜬다.
  await waitFor(() => expect(screen.getByText(/건너뛰/)).toBeInTheDocument())
  // 디코딩이 필요한 재전사·화자 구분은 시도조차 하지 않는다(메모리 폭발 방지).
  expect(transcribeMock).not.toHaveBeenCalled()
  expect(diarizeMock).not.toHaveBeenCalled()
  // 요약은 기존 자막으로 진행된다.
  expect(summarizeMock).toHaveBeenCalled()
  expect((await getSummaries(m.id)).length).toBeGreaterThan(0)
})
...
test('분할 회의 1부를 열면 부 탭이 모두 보이고 통합 요약은 마지막 부 안내가 뜬다', async () => {
  const p1 = await createMeeting()
  await markGroupFirstPart(p1.id, p1.id, p1.title, ' (1부)')
  await appendSegment({ meetingId: p1.id, startSec: 0, endSec: 5, text: '1부 내용입니다', source: 'webspeech', isFinal: true })
  ...
  await saveSummary(p3.id, 'minutes', '## 통합 요약 결과', 'gemini-3.5-flash') // 요약은 마지막 부에만
  await finishMeeting(p3.id, 30)

  renderPage(p1.id)
  // 부 탭 3개 노출
  await waitFor(() => expect(screen.getByRole('button', { name: '1부' })).toBeInTheDocument())
  expect(screen.getByRole('button', { name: '2부' })).toBeInTheDocument()
  expect(screen.getByRole('button', { name: '3부' })).toBeInTheDocument()
  // 1부에는 요약이 없으므로 마지막 부 안내가 뜨고, 통합 요약 본문은 여기 없다
  expect(screen.getByText(/통합 요약은 마지막 부에 있어요/)).toBeInTheDocument()
  expect(screen.queryByText(/통합 요약 결과/)).not.toBeInTheDocument()
  // 3부 탭을 누르면 마지막 부 회의록으로 이동한다
  await userEvent.click(screen.getByRole('button', { name: '3부' }))
  await waitFor(() => expect(screen.getByText(/통합 요약 결과/)).toBeInTheDocument())
})
