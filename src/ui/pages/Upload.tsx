...
import { cleanTranscriptSegments } from '../../core/meetingActions'
...
export default function Upload() {
  ...
  async function start() {
    ...
    try {
      ...
      setStage('저장 중…')
      // 반복·환각 후처리 체인을 적용해 저장한다 — 전부 걸러지면 0개로 저장되어 빈 상태 UI가 안내한다.
      // 등록된 보정 사전도 자동 적용한다.
      await replaceSegments(meetingId, cleanTranscriptSegments(segs).map(s => ({
