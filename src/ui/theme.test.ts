...
test('디자인 토큰 CSS 변수가 정의되어 있다', () => {
  for (const v of ['--bg-base: #EEF1F7', '--accent: #1E43B8',
    '--accent-hover: #16359B', '--text-strong: #0F1222',
    '--rec: #E5484D', '--island: rgba(18, 20, 30, 0.82)']) {
    expect(css).toContain(v)
  }
})

test('구계약 별칭 변수(--bg/--surface/--border)가 유지된다', () => {
  for (const v of ['--bg: var(--bg-base)', '--surface: var(--glass)', '--border: var(--glass-edge)']) {
    expect(css).toContain(v)
  }
})
