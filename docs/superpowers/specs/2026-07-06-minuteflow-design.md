...
## 13. 화자 분리 (2026-07-07 범위 편입 — Plan 3)

리서치 결론(2026-07-07): 브라우저 로컬 화자 분리는 **sherpa-onnx WASM**으로 실현 가능이 검증됨. Gemini BYOK 경로는 탈락 — 1시간급 오디오에서 화자 라벨 일관성이 무너지고(공식 포럼 실측), 20MB 초과 오디오에 필요한 Files API가 브라우저 CORS를 차단해 서버 없이는 불가(서버리스 제약 위반).

- **파이프라인**: sherpa-onnx WASM — pyannote segmentation-3.0 ONNX(~6.6MB) + 3D-Speaker CAM++ 임베딩(~28MB) + agglomerative clustering. 모델 합계 ~35-45MB, 1회 다운로드 후 Cache API 캐시. 공식 브라우저 데모: k2-fsa/web-assembly-speaker-diarization-sherpa-onnx (HF Spaces)
- **화자 수**: 자동 추정(코사인 유사도 임계값) 기본 + 수동 지정 옵션 ("참석자 N명")
- **전사 병합**: WhisperX 패턴 — 전사 세그먼트의 시간 구간과 화자 타임라인의 교집합 최대 화자에 배정. `TranscriptSegment`에 `speaker?: string` 필드 추가
- **UI**: 화자 8색 팔레트(design-tokens.md)로 세그먼트에 화자 배지·아바타 표시, 화자 이름 편집(라벨 → 실명 일괄 치환)
- **제약 고지**: 겹치는 발화 구간은 단일 라벨(손실적) — 수동 교정 가능하게. 임베딩 모델이 영어(VoxCeleb) 학습이라 한국어 화자 구분 정확도는 배포 후 실측 필요. 1시간 오디오는 청크 분할 처리(WASM 메모리 압박 방지)
...
| Plan 3 | **화자 분리** (sherpa-onnx WASM + 병합 + 화자 UI) | 대기 |
